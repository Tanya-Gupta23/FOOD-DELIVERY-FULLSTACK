...
function DeliveryBoy() {
  ...
  return (
    <div className="w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto">
      ...
      <div className="w-full max-w-[800px] flex flex-col gap-10 items-center">
        ...
        <div className="bg-white rounded-2xl shadow-md p-5 flex flex-col justify-start items-center w-[90%] border border-orange-100 text-center gap-3">
          ...
          <p className='text-[#ff4d2d]'>
            <span className='font-semibold'>Latitude:</span>, {deliveryBoyLocation?.lat}, <span className='font-semibold'>Longitude:</span> {deliveryBoyLocation?.lon}
          </p>
              ...
              availableAssignments.map((a, index) => (
                <div className='border rounded-lg p-4 flex justify-between items-center' key={index}>
                  <div>
                    ...
                    <p className='text-sm text-gray-500'> <span className='font-semibold'>DeliveryAddress:- </span>  {a.deliveryAddress.text}</p>


                    <p className='text-xs text-gray-400'>{a.items.length} items| ₹{a.subtotal}</p>
                  </div>
                  <button className="bg-[#ff4d2d] text-white px-4 py-1 rounded-lg text-sm hover:bg-orange-600" onClick={() => acceptOrder(a.assignmentId)}>
                    Accept
                  </button>
        ...
        customerLocation: {
          ...
          {!showOtpBox ?
            ...
            <div className='mt-4 p-4 border rounded-xl bg-gray-50'>
              <p className='text-sm font-semibold mb-2'>Enter the OTP sent to <span className='text-[#ff2d4d]'>{currentOrder.user.fullName}</span></p>
              ...
              <button
                className="w-full bg-[#ff2d4d] hover:bg-[#e62545] text-white py-2 rounded-lg font-semibold transition-all duration-200" onClick={verifyOtp}>
                Submit OTP
              </button>
            </div>
          }
